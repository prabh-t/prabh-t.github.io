...
oneStep (ps,vs,ms) = (new_ps,new_vs,ms)
	where
		...
		unz = unzip x
		x = concat $ y
		y = map (\ts -> updateSegment ts vs ps ms) segments --`using` parList rdeepseq
		!segments = map (\t -> [t..(min (t + chunksize - 1) n)]) [1,1+chunksize .. n] --`using` parList rdeepseq
		chunksize = n `quot` (toInteger numCapabilities * 30)

updateSegment ts vs ps ms = map (updateVel vs ps ms) ts
